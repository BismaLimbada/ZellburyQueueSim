...
public class StatisticsService
{
    ...
    /// <summary>
    /// Bins a sample into <paramref name="binCount"/> equal-width bins and
    /// returns bin labels, observed counts, and the bin edges (so a fitted
    /// theoretical curve can be evaluated over the same edges).
    /// </summary>
    public static (string[] Labels, int[] Counts, double[] Edges) Histogram(IReadOnlyList<double> sample, int binCount)
    {
        if (sample.Count == 0 || binCount < 1)
            return (Array.Empty<string>(), Array.Empty<int>(), Array.Empty<double>());

        double min = sample.Min();
        double max = sample.Max();
        double width = Math.Max((max - min) / binCount, 0.01);

        var edges = new double[binCount + 1];
        for (int i = 0; i <= binCount; i++) edges[i] = min + i * width;

        var counts = new int[binCount];
        foreach (var x in sample)
        {
            int idx = Math.Min((int)((x - min) / width), binCount - 1);
            counts[idx]++;
        }

        var labels = new string[binCount];
        for (int i = 0; i < binCount; i++) labels[i] = $"{edges[i]:0.0}-{edges[i + 1]:0.0}";

        return (labels, counts, edges);
    }

    /// <summary>
    /// Expected count per bin under a fitted Exponential(rate) distribution,
    /// for overlaying a density curve on an observed histogram so the reader
    /// can visually judge the goodness of fit.
    /// </summary>
    public static double[] ExponentialExpectedCounts(double[] edges, double rate, int sampleSize)
    {
        if (edges.Length < 2) return Array.Empty<double>();
        var result = new double[edges.Length - 1];
        for (int i = 0; i < edges.Length - 1; i++)
        {
            double pLower = 1 - Math.Exp(-rate * edges[i]);
            double pUpper = 1 - Math.Exp(-rate * edges[i + 1]);
            result[i] = (pUpper - pLower) * sampleSize;
        }
        return result;
    }
