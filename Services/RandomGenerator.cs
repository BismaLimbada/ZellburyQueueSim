...
public class RandomGenerator
{
    ...
    /// <summary>
    /// Same empirical inverse-transform sampling as <see cref="NextEmpirical"/>,
    /// but the draw is rescaled from the sample's own mean to a target mean.
    /// Uniform rescaling preserves the coefficient of variation (and hence
    /// Ca^2/Cs^2) of the original data exactly, so this lets G/G/1 explore
    /// different arrival/service rates while still replaying the real
    /// shape of the recorded Zellbury distribution rather than an assumed one.
    /// </summary>
    public double NextEmpiricalScaled(IReadOnlyList<double> sortedSample, double sampleMean, double targetMean)
    {
        double raw = NextEmpirical(sortedSample);
        if (sampleMean <= 0) return raw;
        double scaleFactor = targetMean / sampleMean;
        return Math.Max(raw * scaleFactor, 0.01);
    }
}
